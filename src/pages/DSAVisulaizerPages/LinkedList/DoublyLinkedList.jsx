...
const DoublyLinkedListVisualizer = () => {
  ...
  // A function to create a new node object with unique, random hex addresses
  ...
  const handleAppend = async () => {
    ...
    setList(prevList => {
      const newList = [...prevList, { ...newNode, isAdding: true }];
      if (newList.length > 1) {
        newList[newList.length - 1].prev = newList[newList.length - 2].address;
      }
      return newList;
    });
  ...
  const handlePrepend = async () => {
    ...
    setList(prevList => {
      const newList = [{ ...newNode, isAdding: true }, ...prevList];
      if (newList.length > 1) {
        newList[1].prev = newList[0].address;
      }
      return newList;
    });
  ...
  const handleInsertAtIndex = async () => {
    ...
    setList(prevList => {
      ...
      // Update pointers
      if (index > 0) {
        newList[index].prev = newList[index - 1].address;
      }
      if (index < newList.length - 1) {
        newList[index + 1].prev = newList[index].address;
      }
      return newList;
    });
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-gray-100 flex flex-col items-center justify-center p-4">
      <div className="bg-gray-800 p-8 rounded-xl shadow-2xl w-full max-w-5xl">
        <h1 className="lg:text-4xl md:text-3xl sm:text-2xl text-xl font-extrabold text-white text-center mb-6">Doubly Linked List Visualizer</h1>
        <p className="lg:text-lg md:text-md text-base text-gray-400 text-center mb-8">
          A Doubly Linked List uses nodes with `next` and `prev` pointers.
        </p>
        ...
        <div className="mt-8 flex items-center justify-start overflow-x-auto p-4 border border-gray-700 rounded-lg">
          ...
          {list.map((node, index) => (
            <React.Fragment key={node.key}>
              ...
              >
                ...
                <div className="text-sm text-gray-200 mt-2">Prev: <span className="font-bold text-white">{index > 0 ? list[index - 1].address : 'Null'}</span></div>
                <div className="text-sm text-gray-200 mt-1">Next: <span className="font-bold text-white">{index < list.length - 1 ? list[index + 1].address : 'Null'}</span></div>
              </div>
              {index < list.length - 1 && (
                <div className="flex items-center mx-2">
                  <div className="relative w-12 h-1 bg-gray-400">
                    ...
                    <div className="w-0 h-0 border-t-[6px] border-t-transparent border-b-[6px] border-b-transparent border-r-[12px] border-r-gray-400 absolute top-1/2 left-0 -translate-y-1/2 -translate-x-1"></div>
                  </div>
                </div>
              )}
            </React.Fragment>
          ...
          {list.length === 0 && (
            <div className="flex flex-col items-center mx-auto text-gray-500 text-xl w-full">
              <span className="md:text-sm text-xs font-bold text-green-500">HEAD</span>
              <div className="w-16 h-8 text-center leading-8 bg-green-500 rounded-lg shadow-lg mt-2"></div>
          ...
          {list.length > 0 && (
            <div className="flex flex-col items-center ml-6">
              <span className="md:text-sm text-xs font-bold text-red-500">TAIL</span>
